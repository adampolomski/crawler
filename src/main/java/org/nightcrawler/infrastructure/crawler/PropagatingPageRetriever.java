...
public class PropagatingPageRetriever extends PageRetriever {
	...
	public void crawl(final URI uri, final Consumer<Page> handler) {
		index.aquire(uri).ifPresent(writer -> delegateRetriever.crawl(uri, propagatingConsumer(handler, writer)));
	}

	private Consumer<Page> propagatingConsumer(final Consumer<Page> handler, final Consumer<Page> writer) {
		return page -> {
			writer.accept(page);
			handler.accept(page);
			page.visitLinks(link -> crawl(link, handler));
		};
	}	
}
